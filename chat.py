...
from config import MEM_EXTRACTION_PROMPT, MEMORY_USE_PROMPT, ROLEPLAY_PROMPT  # 假设这些常量已定义
...
def chat_one(
    ...
    try:
        ...
        if has_mem:
            logging.info(f"保存记忆到数据库: {memory}")
            save_to_db(memory)  # 保存到数据库
        else:
...
# 输入：content（用户的输入），top_k（返回的最相似记忆数量），threshold（相似度阈值）
# 输出：最相似的记忆和相似度
