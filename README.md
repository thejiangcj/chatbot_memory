...
## 项目概述
本项目旨在开发一个结合多模态内容记忆功能的角色扮演对话系统。系统能从对话中抽取用户的偏好（提取记忆），并在后续对话中利用这些记忆提供更加个性化、更拟人的回复。
...
注意 1：Moonshot有每分钟请求不超过3次的Rate Limit限制，最好不要连续使用，会强制重试然后很慢。
Deepseek因为用的人爆满，所以API也很慢，请耐心等待一下结果输出。

注意 2：Deepseek 没有视觉模型 API，因此目前只支持 Moonshot 的 VLM API。
...
实现：通过 VLM+Prompt 实现图片和文字的对话，同时将图片的描述文字的记忆进行存储。图片记忆的匹配和文本记忆匹配一致（当前方案）。
...
再或者，是bot对用户说”我爱你“，这也可以作为一个记忆”bot对用户说了第一次‘我爱你’“。这种是否也需要记忆呢？
或者用户问bot，“我们改天去公园约会吧？”。bot表示同意。
那么这样可以提取为“用户和bot约定了去公园约会”。我感觉这也是个很有意思的点，值得后续迭代过程中探索。
